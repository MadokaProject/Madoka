...
async def process(
    ...
    async def master_admin_process(_: Union[Friend, Member]):
        if install := subcommand.get("install"):
                    ...
                    await app.send_message(
                        ...
                        MessageChain(
                            [
                                ...
                                Plain(
                                    "\n".join(
                                        f"{i + 1}. {p['name']} - {p['author']}: {p['version']}"
                                        for i, p in enumerate(plugins)
                                    )
                                ),
                                Plain("\n是否要继续安装（是/否）"),
            ...
            await app.send_message(
                sender,
                MessageChain(f"正在尝试安装插件: {install_plugin['name']} - {install_plugin['author']}"),
            )
            ...
            await app.send_message(
                sender,
                MessageChain(f"正在尝试加载插件: {load_plugin['name']} - {load_plugin['author']}"),
            )
            ...
            await app.send_message(
                sender,
                MessageChain(f"正在尝试卸载插件: {unloaded_plugin['name']} - {unloaded_plugin['author']}"),
            )
            ...
            if plugin == "all_plugin":
                ...
                await app.send_message(
                    sender,
                    MessageChain(f"正在尝试重载插件: {reload_plugin['name']} - {reload_plugin['author']}"),
                )
            ...
            if msg := await plugin_mgr.check_update():
                return MessageChain([Plain("检测到插件更新"), Image(data_bytes=msg)])
