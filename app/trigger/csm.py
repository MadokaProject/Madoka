...
class CSM(Trigger):
    ...
    async def process(self):
        ...
        if self.target.id in BANNED_USER:
            self.as_last = True
        if (
            not isinstance(self.sender, Group)
            or Permission.manual(self.target, Permission.GROUP_ADMIN)
            or self.sender.account_perm == MemberPerm.Member
        ):
            return
        if (
    ...
    async def spam(self):
        try:
            res = (
                DBMsg.select()
                .where(DBMsg.uid == self.sender.id, DBMsg.qid == self.target.id)
                .order_by(DBMsg.id.desc())
                .limit(3)
            )  # 获取该群组某人的最后三条消息记录
            if res.count() > 2:
                ...
                if time < flood["time"]:  # 刷屏禁言
                    ...
                    resp = MessageChain([At(self.target.id), Plain(" " + flood["message"])])
                    await self.do_send(resp)
                    return True
                elif (
                    all(MessageChain.from_persistent_string(i.content) == temp_content for i in res)
                    and time < duplicate["time"]
                ):  # 发送重复消息禁言
                    await self.app.mute_member(self.sender, self.target, duplicate["mute"])
                    resp = MessageChain([At(self.target), Plain(" " + duplicate["message"])])
                    await self.do_send(resp)
                    return True
            ...
            if len(self.message.display) > over_length["text"]:
                ...
                resp = MessageChain([At(self.target.id), Plain(" " + over_length["message"])])
                await self.do_send(resp)
                return True
